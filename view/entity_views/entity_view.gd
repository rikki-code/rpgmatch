...
## it. `start` can sit inside a different segment than the one it's landing
## in (e.g. above a pit), so the entity must stay hidden until it reaches
## `reveal` — only from there to `target` should it actually be shown.
## `owner_node` is only used to open a Tween (needs a live scene node).
func play_spawn(_node: Node3D, _start: Vector3, _reveal: Vector3, _target: Vector3, _owner_node: Node) -> Tween:
	return null
